...
class UserController extends AppController {
    ...
    */
    function index() {
        ...
        {
            $this->Session->setFlash(__('Utilizador inválido.'), 'error');
            $this->redirect('/');
        }
        ...
        if (!empty($this->request->data))
        {
            ...
            {// password changed
                ...
                else
                {
                    ...
                    $this->Session->setFlash(__('Password alterada!'), 'success');
                }
            }
            ...
            {//Check if that email is not registered by another user
                ...
                {
                    $this->Session->setFlash(__('Este e-mail já está a ser utilizado por outro utilizador, por favor tente outro email!'), 'error');
                ...
                $email->subject(sprintf(__('O seu pedido de alteração de e-mail em %s '), Configure::read('Authake.service')));
            ...
            if ($this->User->save($user['User']))
            {
                ...
                {
                    case 1:
                    $this->Session->setFlash(__('O seu email foi alterado, deverá receber um email com as instruções de confirmação.'), 'warning');
                    break;
                    case 2:
                    $this->Session->setFlash(sprintf(__('Erro ao enviar email de reposição da password. Por favor contacte o administrador em %s'), Configure::read('Authake.systemReplyTo')), 'error');
                    break;
                    default:$this->Session->setFlash(__('O perfil foi guardado.'), 'success');
                }
            }
