...
class AuthakeComponent extends Component {
    ...
    function startup(Controller $controller = null) {
        App::uses('PhpReader', 'Configure');
	      Configure::config('Authake', new PhpReader(APP.'Config/'));
        ...
        if (Configure::read('Authake.loginAction') == null) {
            Configure::write('Authake.loginAction', array('controller' => 'user', 'action' => 'login', 'admin' => 0));
        }
        /**
        ...
        if (Configure::read('Authake.defaultDeniedAction') == null) {
            Configure::write('Authake.defaultDeniedAction', array('controller' => 'user', 'action' => 'denied', 'admin' => 0));
        }
        /**
    ...
    function beforeFilter(&$controller) { //pr($this);
        ...
        if ($tm && $this->isLogged()) {
            ...
            if ((time() - $ts) > $tm) {
                ...
                $this->logout();
                $this->Session->setFlash(__('A sua sessão expirou!'), 'warning');
        ...
        if (!$this->isAllowed($path)) { // check for permissions
            ...
            $this->Session->setFlash(sprintf(__('Tem de efectuar login para aceder a %s'), $strpath), 'warning');
       $controller->redirect($loginAction);
            }
            $this->_flashmessage = '';
        }
